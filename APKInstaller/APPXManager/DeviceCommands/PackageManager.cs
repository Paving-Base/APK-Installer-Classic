...
namespace APPXManager.DeviceCommands
{
    ...
    public static class PackageManager
    {
        ...
        /// </summary>
        public static IEnumerable<PackageInfo> GetPackages(string? arg = null)
        {
        ...
        /// Find the packages.
        /// </summary>
        public static (bool isfound, IEnumerable<PackageInfo> info) FindPackageByName(string name)
        {
            PackageManagerReceiver pmr = new PackageManagerReceiver();
            CommandHelper.ExecuteShellCommand($"{FindByName}{name}", pmr);
            if (pmr.Packages.Count <= 0) { return (false, new ObservableCollection<PackageInfo>()); }
            else { return (true, pmr.Packages); }
        }
